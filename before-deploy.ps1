...
if (Enter-OncePerDeployment "install_docker_image")
{
	...
	docker run -d -e "tokens=$env:TOKENS" -p 80:80/tcp --name=CSDiscord --net=nat --net-alias=CSDiscord --hostname=CSDiscord --restart=always --memory=2g "cisien/csdiscord:$tag"
}