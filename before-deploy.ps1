...
if (Enter-OncePerDeployment "install_docker_image")
{
	...
	docker rm CSDiscord
	docker run -d -e "tokens=$env:TOKENS" -e "log_webhook_token=$env:LOG_WEBHOOK_TOKEN" -p 31337:80/tcp --name=CSDiscord --net=nat --net-alias=CSDiscord --hostname=CSDiscord --restart=always --memory=4g --health-interval=2m --isolation hyperv "cisien/csdiscord:$tag"
}
